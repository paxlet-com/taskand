...
export function registry(action, payload = {}, timeout_ms = 60000) {
  const r = spawnSync('node', [REGISTRY], { input: JSON.stringify({ action, ...payload }), encoding: 'utf8',
    timeout: timeout_ms + 5000, killSignal: 'SIGKILL', maxBuffer: 32 * 1024 * 1024,
    env: { PATH: process.env.PATH, HOME: process.env.HOME, TASKAND_CALL_DEPTH: process.env.TASKAND_CALL_DEPTH || '0' } });
  try { return JSON.parse(r.stdout); }
  catch { return { ok: false, errorType: 'REGISTRY_ERROR', error: r.error?.message || r.stderr || 'Brak odpowiedzi rejestru' }; }
}
