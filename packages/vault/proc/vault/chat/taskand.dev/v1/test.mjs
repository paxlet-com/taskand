...
const good = spawnSync('node', [join(dir, 'bin.mjs')], { input: JSON.stringify({ message: "migruj token llm do vaulta" }) });
if (good.status !== 0) {
  console.error('Test 2 failed: oczekiwano exit 0', good.stderr);
  process.exit(1);
}
const out = JSON.parse(good.stdout);
if (!out.ok || !out.reply || out.intent !== "vault-migrate") {
  console.error('Test 2 failed: niepoprawna odpowiedź chat', out);
  process.exit(1);
}

console.log('proc://taskand.dev/vault/chat/v1: kontrakt ✓');
